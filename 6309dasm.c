...
unsigned Dasm6309 (char *buffer, int pc, unsigned char *memory, size_t memory_size, int memory_base)
{
    ...
    *buffer = '\0';
        ...
        if( pg1opcodes[i].mode >= PG2 )
        {
            ...
            {   /* opcode found */
                ...
                for (j = 0; j < numoperands; j++)
                    operandarray[j] = memory[(pc++)-memory_base];
    ...
    case IND:     /* indirect- many flavors */
        ...
        {   /* 8-bit offset */
            ...
            offset = (INT8)memory[(pc++)-memory_base];
            p++;
        ...
        {   /* 16-bit */
            ...
            {
                offset = (INT16)( (memory[(pc++)-memory_base] << 8) );
                offset += memory[(pc++)-memory_base];
            }
...
//      DBGDASM.dst_ea_size = size;
...
#endif
        {
            const char *pSymbol = getSymbol(value + size);
            if (pSymbol)
                sprintf( buffer[which], "%-12s\t($%04x)", pSymbol, value + size );
            else
            {
                add_code_label( value + size );
                sprintf( buffer[which], "%04x", value + size );
            }
        }
    ...
    {
        const char *pSymbol = getSymbol(result);
        if (pSymbol)
            sprintf( buffer[which], "%-12s\t(%s$%0*X)", pSymbol, sign, width, result );
        else
        {
